...
void move_arm(X7StateInterface& controller, ros::NodeHandle& nh,
  ...
  */
  ...
  mat_to_xyzrpy(left_grasp_pose, left_grasp_xyz, left_grasp_rpy);
  mat_to_xyzrpy(right_grasp_pose, right_grasp_xyz, right_grasp_rpy);
  //mat_to_xyzrpy(left_goal_pose, left_goal_xyz, left_goal_rpy);
  //mat_to_xyzrpy(right_goal_pose, right_goal_xyz, right_goal_rpy);

  // move to grasp pose
  std::vector<double> left_grasp = {left_grasp_xyz[0], left_grasp_xyz[1], left_grasp_xyz[2],left_quat_rpy[0], left_quat_rpy[1], left_quat_rpy[2]};
  std::vector<double> right_grasp = {right_grasp_xyz[0], right_grasp_xyz[1], right_grasp_xyz[2], right_quat_rpy[0], right_quat_rpy[1], right_quat_rpy[2]};
  ...
  get_current_poses(controller, curr_left, curr_right);
...
/*
  // move to gaol pose
  std::vector<double> left_goal = {left_goal_xyz[0], left_goal_xyz[1], left_goal_xyz[2],left_quat_rpy[0], left_quat_rpy[1], left_quat_rpy[2]};
  std::vector<double> right_goal = {right_goal_xyz[0], right_goal_xyz[1], right_goal_xyz[2], right_quat_rpy[0], right_quat_rpy[1], right_quat_rpy[2]};
  std::thread left_goal_thread(&X7StateInterface::set_ee_pose_cmd, &controller, std::ref(nh), true, left_goal, 0.0);
  std::thread right_goal_thread(&X7StateInterface::set_ee_pose_cmd, &controller, std::ref(nh), false, right_goal, 0.0);
  left_goal_thread.join();
  right_goal_thread.join();
  sleep_sec(1.0);
  
  get_current_poses(controller, curr_left, curr_left);
  // push the lid
  ...
  std::vector<double> left_push = {
    left_goal[0], left_goal[1], left_goal[2] - press_offset,
    left_goal[3], left_goal[4], left_goal[5]
  };
 

  std::vector<double> right_push = {
    right_goal[0], right_goal[1], right_goal[2] - press_offset,
    right_goal[3], right_goal[4], right_goal[5]
  };
